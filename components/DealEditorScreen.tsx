...
interface DealEditorScreenProps {
    onBack: () => void;
...
const DealEditorScreen: React.FC<DealEditorScreenProps> = ({ onBack, onCreateDeal, onUpdateDeal, dealToEdit }) => {
    const isEditing = !!dealToEdit;

    const [name, setName] = useState(dealToEdit?.name || '');
    const [originalPrice, setOriginalPrice] = useState(dealToEdit ? formatPrice(dealToEdit.originalPrice) : '');
    const [salePrice, setSalePrice] = useState(dealToEdit ? formatPrice(dealToEdit.salePrice) : '');
    const [remaining, setRemaining] = useState(dealToEdit?.remaining.toString() || '');
    const [description, setDescription] = useState(dealToEdit?.description || '');
    const [imagePreview, setImagePreview] = useState<string | null>(dealToEdit?.images[0] || null);
        ...
        <div className="absolute inset-0 bg-gray-50 z-20">
            <header className="p-4 bg-white shadow-sm sticky top-0 z-10 flex items-center">
                ...
                <h1 className="text-xl font-bold text-center text-gray-800">
                    {isEditing ? 'Chỉnh sửa túi giải cứu' : 'Tạo túi giải cứu mới'}
                </h1>
            </header>
            ...
            <main className="p-4">
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="pt-4">
                         <button type="submit" className="w-full bg-green-600 text-white font-bold py-3.5 rounded-full shadow-lg">
                            {isEditing ? 'Lưu thay đổi' : 'Lưu và Đăng bán'}
                         </button>
                    </div>
                </form>
            </main>
...
export default DealEditorScreen;